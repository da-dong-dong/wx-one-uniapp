	...
	<view class="bigBox">
		<view class="listInfo">
			<view class="mainBox">
				...
				<view class="listBox">
					<view class="list">
						<view class="text">看板备注：</view>
						<view class="info arr">{{item.remark?item.remark:'无'}}</view>
					</view>	
				</view>

				<view class="butBox" v-if="item.processStatus !== 'COMPLETE'">
					<view v-if="flag" class="but marginL20" :class="(!fnPermissionShopId('STORE_CONTROL_WATCH_UPDATE'))?'':'fn_back'" @click="cancel(item.id)">取消档期</view>
					<view v-if="flag" class="but marginL20" :class="(!fnPermissionShopId('STORE_CONTROL_WATCH_CANCEL'))?'':'fn_back'" @click="change(item)">修改档期</view>
				</view>
			</view>
		</view>
