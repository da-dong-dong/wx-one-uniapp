		...
		<view class="bigBox">
			<view class="listInfo" v-for="(item,index) in listInfo" :key="item.id">
				...
				<view class="butBox" v-if="item.id && item.processStatus !== 'COMPLETE'">
					<view :class="(!fnPermissionShopId('STORE_CONTROL_WATCH_UPDATE'))?'':'fn_back'" class="but" @click="updata1(item.id)">修改看板档期</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			updata1(id){
				//判断修改档期权限
				if(this.fnPermissionShopId('STORE_CONTROL_WATCH_UPDATE')){
					$Message({
						content: '权限不足',
						type: 'error'
					});
			...
			updata(id){
				...
				uni.navigateTo({
					url:`./kbAddorUpdata/kbAddorUpdata?id=${id}&reservationDate=${reservationDate}&reservationShopId=${reservationShopId}&reservationTime=${reservationTime}&typographyTypeId=${typographyTypeId}`
				})
			},
			// 打开取消档期模态框
			cancel(id){
				//判断取消档期权限
				if(this.fnPermissionShopId('STORE_CONTROL_WATCH_CANCEL')){
					$Message({
						content: '权限不足',
						type: 'error'
					});
